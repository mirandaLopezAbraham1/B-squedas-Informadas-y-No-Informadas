...
class ArbolIDDFS:
    def __init__(self):
        ...
        self.arbol = None  # Mantenemos un solo árbol
    ...
    def imprimir_arbol(self, nodo):
        ...
        cola = deque()
        cola.append((nodo, None))

        nivel_actual = 0  # Para rastrear el nivel actual en el árbol
        while cola:
            nodo_actual, padre = cola.popleft()  # Desencolamos el nodo y su padre
    ...
    def busquedaIterativa(self, limite):
        # Creamos el árbol y las posiciones visitadas una sola vez
        self.arbol = Arbol()
        self.visitadas = set()

        # Comienza con un límite de profundidad de 0 y lo incrementa en cada iteración
        for profundidad in range(limite + 1):
            print(f"Buscando con profundidad límite {profundidad}...")

            # Reseteamos el contador de nodos en la ruta de camino
            self.contador_nodos = 0

            # Verificamos si ya encontramos la solución antes de iniciar la búsqueda
            if self.esObj:
                break  # Salimos del bucle si ya encontramos la solución

            raiz_a = self.mapa[3][2]
            self.arbol.raiz = Nodo(self.mapa)
            self.visitadas.clear()  # Limpiamos el conjunto de posiciones visitadas
            self.visitadas.add(tuple(map(tuple, self.mapa)))  # Marcar la posición inicial como visitada
            self.ejecutar_estados_bfs(self.arbol.raiz, profundidad)

            # Si se encuentra la solución, imprimir y salir del bucle
            if self.esObj:
                self.imprimir_arbol(self.arbol.raiz)
                break  # Salimos del bucle si encontramos la solución


    def ejecutar_estados_bfs(self, nodo, limite):
        if limite == 0 or self.esObj:  # Si ya encontraste la solución o alcanzaste el límite, detén la expansión.
            return
        hoja = nodo.dato

        # Creamos una cola para la búsqueda BFS
        cola = deque()
        cola.append(nodo)

        while cola:
            nodo_actual = cola.popleft()  # Desencolamos el nodo actual

            # Realizamos los movimientos y expandimos los nodos
            self.mover_derecha(nodo_actual)
            self.mover_izquierda(nodo_actual)
            self.avanzar_recto(nodo_actual)
            self.mover_atras(nodo_actual)

            for hijo in nodo_actual.hijos:
                cola.append(hijo)

                if self.esObj:  # Si se encontró una solución, no continuar expandiendo nodos.
                    return

                # Realizamos los movimientos y expandimos los nodos
                self.mover_derecha(hijo)
                self.mover_izquierda(hijo)
                self.avanzar_recto(hijo)
                self.mover_atras(hijo)

    def mover_derecha(self, nodo):
...
#@profile
def main():
    arbol_iterativo = ArbolIDDFS()
    profundidad_maxima = 5  # Ajusta la profundidad máxima según tus necesidades
    arbol_iterativo.busquedaIterativa(profundidad_maxima)
    # Llama a la función para imprimir el árbol con Graphviz
    arbol_iterativo.imprimir_arbol_con_graphviz("arbolIterativo")
...
if __name__ == "__main__":
    ...
    print(f"Tiempo de ejecución por épocas: {elapsed_time} segundos")