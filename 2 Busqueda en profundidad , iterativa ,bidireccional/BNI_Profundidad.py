...
class ArbolGeneral:
    ...
    def imprimir_arbol(self):
        ...
        # Usamos DFS para recorrer el árbol e imprimir los nodos
        pila = []
        pila.append((self.arbol.raiz, None))

        nivel_actual = 0  # Para rastrear el nivel actual en el árbol
        while pila:
            nodo_actual, padre = pila.pop()  # Desempaquetamos el nodo y su padre
        ...
        for i in range(4):
            for j in range(3):
                if solucion[i][j] == "X" or solucion[i][j] == "UTA":
                    ruta += self.mapa[i][j] + "->"
                    self.contador_nodos += (
                        1  # Incrementar el contador de nodos en la ruta de camino
                    )
        ruta += "MALL"
        return ruta
    ...
    def crear_arbol(self):
        ...
        self.busquedaProfundidad(self.arbol.raiz)
    ...
    def busquedaProfundidad(self, nodo):
        # Usamos una pila en lugar de una cola para implementar DFS
        pila = []
        pila.append(nodo)

        while not self.esObj and pila:
            nodo_actual = pila.pop()
            hoja = nodo_actual.dato

            # Realizamos los movimientos y expandimos los nodos
            self.mover_derecha(nodo_actual, pila)
            self.mover_izquierda(nodo_actual, pila)
            self.avanzar_recto(nodo_actual, pila)
            self.mover_atras(nodo_actual, pila)

        if self.esObj:
            self.imprimir_arbol()
            print("")

    def mover_derecha(self, nodo, pila):
        ...
        if fila > 0 and aux[fila - 1][columna] not in self.visitadas:
            ...
            if FO != "X":  # Evitar nodos con "X"
                if FO == "MALL":
                    ...
                    pila.append(Nodo(aux))

    def mover_izquierda(self, nodo, pila):
        aux_posicion = self.obtener_posicion_actual(nodo.dato)
        fila = aux_posicion[0]
        columna = aux_posicion[1]
        padre = nodo.dato
    ...
    def avanzar_recto(self, nodo, pila):
        aux_posicion = self.obtener_posicion_actual(nodo.dato)
        fila = aux_posicion[0]
        columna = aux_posicion[1]
        padre = nodo.dato
    ...
    def mover_atras(self, nodo, pila):
        aux_posicion = self.obtener_posicion_actual(nodo.dato)
        fila = aux_posicion[0]
        columna = aux_posicion[1]
        padre = nodo.dato
...
def main():
    ag2 = ArbolGeneral()
    ag2.crear_arbol()
    ag2.imprimir_arbol_con_graphviz(
        "arbolProfundidad"
